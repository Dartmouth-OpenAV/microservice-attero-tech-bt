...
func setFrameworkGlobals() {
	...
	framework.MicroserviceName = "OpenAV Attero Tech unD6IO-BT Bluetooth Microservice"
	framework.DefaultSocketPort = 49494 // AT's default socket port
...
func doDeviceSpecificSet(socketKey string, setting string, arg1 string, arg2 string, arg3 string) (string, error) {
	...
	switch setting {
	case "pair":
		return setPairing(socketKey, arg1)
	case "clear":
		return setClear(socketKey, arg1)
	}
...
func doDeviceSpecificGet(socketKey string, setting string, arg1 string, arg2 string) (string, error) {
	...
	switch setting {
	case "pair":
		return getPairing(socketKey)
	case "status":
		return getStatus(socketKey)
	case "devicename":
		return getDeviceName(socketKey)
	case "musicinfo":
		return getMusicInfo(socketKey)
	}
